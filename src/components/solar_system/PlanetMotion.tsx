...
function nextState(pos: number[], nparam: number[], param: number[]){
  let x = pos[0], y = pos[1], e = nparam[0], a = nparam[1];
  let pe = param[0], pa = param[1];
  ...
  if(pe != e || pa != a){ // ellipse changed
    let r_1 = k / (1 + e * Math.cos(T_0));
    [x_res, y_res] = [r_1 * Math.cos(T_0), r_1 * Math.sin(T_0)];
  }else{
...
export default function PlanetMotion(props: PlanetMotionProp){
  ...
  const {ec, axis, offsetX, offsetY} = props;
  const [coor, setCoor] = useState([100, 100]);
  const [precoor, setPrecoor] = useState([0, 0]);
  const [param, setParam] = useState([ec, axis])
  ...
  return (
    <motion.span
      ...
      style={ballStyle}
      animate={{
        x: [precoor[0] + offsetX, coor[0] + offsetX], 
        y:[precoor[1] + offsetY, coor[1] + offsetY]
      }}
      transition={{
      ...
      onAnimationComplete={() => {
        setPrecoor(coor);
        setCoor(nextState(coor, [ec, axis], param));
        setParam([ec, axis]);
      }}
    >
    </motion.span>
